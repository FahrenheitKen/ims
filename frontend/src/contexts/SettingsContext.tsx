...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setCommissionRate(typeof data.commission_rate === 'number' ? data.commission_rate : 1);
    } catch {
      // Keep defaults
    } finally {
      setLoading(false);
  ...
  return (
    <SettingsContext.Provider value={{ appName, appLogo, logoUrl, whatsappNumber, commissionRate, loading, refresh }}>
      {children}
